...
EMOTION_SHORT_DESCRIPTIONS = {
    "страх": "Страх часто з’являється, коли щось сприймається як загроза або небезпека. У тілі це може відчуватись як завмирання, холод, напруга або бажання відступити.",
    "тривога": "Тривога зазвичай пов’язана з невизначеністю та напруженим очікуванням. У тілі це може бути стиснення, поверхневе дихання, напруга в грудях або животі.",
    "гнів": "Гнів часто виникає, коли порушуються межі або щось здається несправедливим. У тілі це може відчуватись як жар, напруга, тиск або потреба діяти.",
    "сум": "Сум часто пов’язаний із втратою, виснаженням або потребою побути в тиші. У тілі може відчуватись як тяжкість, м’якість, втома або сльози.",
    "радість": "Радість пов’язана з відчуттям живості, тепла і контакту. У тілі це може бути легкість, розширення, тепло, більше енергії.",
    "сором": "Сором часто пов’язаний із вразливістю, страхом осуду або бажанням сховатися. У тілі може відчуватись як стискання, жар в обличчі, бажання зменшитися.",
    "провина": "Провина часто з’являється, коли здається, що було зроблено щось не так. У тілі це може бути важкість, тиск, внутрішнє стискання.",
    "інтерес": "Інтерес пов’язаний із допитливістю, увагою і потягом до нового. У тілі це може відчуватись як оживлення, легка активація, зібраність.",
}
...
def dictionary_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="Типи чутливості")],
            [KeyboardButton(text="Головне меню")],
        ],
        resize_keyboard=True
    )
...
async def main():
    ...
    async def body_to_emotion(message: Message):
        await message.answer(
            "Кілька м’яких прикладів:\n\n"
            "Стиснення в грудях\n"
            "— часто може з’являтись поруч із тривогою, страхом або напругою\n\n"
            "Тяжкість у тілі\n"
            "— може бути пов’язана з втомою, смутком або перевантаженням\n\n"
            "Порожнеча всередині\n"
            "— іноді з’являється поруч із самотністю, виснаженням або безсиллям\n\n"
            "Тепло і розширення\n"
            "— може бути поруч із полегшенням, радістю, близькістю\n\n"
            "Цей розділ ми ще поглибимо в наступному етапі.",
            reply_markup=dictionary_menu()
        )
    ...
    @dp.message(F.text.in_(list(EMOTION_DICTIONARY_STAGE1.keys())))
    async def show_emotion_category(message: Message):
        items = EMOTION_DICTIONARY_STAGE1[message.text]
        await message.answer(
            f"{message.text}:",
    ...
    @dp.message(F.text.in_(
        sum([v for v in EMOTION_DICTIONARY_STAGE1.values()], [])
    ))
    async def show_emotion_item(message: Message):
        raw = message.text
        base_word = raw.split(":")[0].strip().lower()

        description = EMOTION_SHORT_DESCRIPTIONS.get(
            base_word,
            "Цей розділ ми ще допрацьовуємо. На наступному етапі тут буде глибший опис: що це за емоція чи почуття, як воно може відчуватись у тілі і про що може бути."
        )
